...
class ForceEdgeBundling():
    ...
    def apply_spring_force(self, edge_idx, i, kP):
        ForceFactors = namedtuple('ForceFactors', 'x y')
    ...
    def forcebundle(self):
        ...
        for cycle in tqdm(range(0, self.C), unit='cycle'):
            for iteration in tqdm(range(math.ceil(I)), unit='Iteration', leave=False):
                forces = {}
                for edge in tqdm(self.data_edges.keys(), unit='Edge (forces)', leave=False):
                    forces[edge] = self.apply_resulting_forces_on_subdivision_points(edge, P, S)
                for e in tqdm(self.data_edges.keys(), unit='Edge (subdivision)', leave=False):
                    for i in range(P):
